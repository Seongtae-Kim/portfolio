...
class Articles:
    ...
    def save_check(self, savepoint=100):
        if len(self.list) % savepoint == 0:
            import json
            with open("./articles/articles.json", "w", encoding='UTF-8-sig') as js:
                dmp=self.list.copy()
...
class Crawler:
    ...
    def articles_loader(self):
        ...
        if not os.path.exists("./articles/articles.json"):
            ...
            self.articles = Articles()
            self.articles.list = json.load(open("./articles/articles.json", "r", encoding="UTF-8-sig"))
            self.checkpoint = int(open("./articles/checkpoint", "r").read())
    ...
    def save_checkpoint(self):
        with open("./articles/checkpoint", "w") as writer:
    ...
    def save_errors(self, url):
        ...
        import pickle
        with open("./articles/error_urls.pkl","wb") as f:
...
import pickle
urls = pickle.load(open("/home/seongtae/SynologyDrive/SIRE/Projects/Crawler/Coronavirus Crawler/articles_url.pkl", "rb"))
print("{:,} source urls...".format(len(urls)))


# For parallel crawling
import math
index = math.floor(len(urls) * (1/4)) # First part
url_keys = list(urls.keys())[:index]

with open("./articles/parallel/urls_1.pkl", "wb") as w:
    dmp = url_keys.copy()
    pickle.dump(dmp, w)
    dmp=None
...
urls = tmp.copy()
tmp=None
print("{:,} urls - PARALLEL [1]".format(len(urls.keys())))
