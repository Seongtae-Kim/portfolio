...
class Articles:
    ...
    def save_check(self, savepoint=100):
        if len(self.list) % savepoint == 0:
            import json
            with open("./articles/articles3.json", "w", encoding='UTF-8-sig') as js:
                dmp=self.list.copy()
...
class Crawler:
    ...
    def articles_loader(self):
        import os, json
        if not os.path.exists("./articles/articles3.json"):
            ...
            self.articles = Articles()
            self.articles.list = json.load(open("./articles/articles3.json", "r", encoding="UTF-8-sig"))
            self.checkpoint = int(open("./articles/checkpoint3", "r").read())
    ...
    def save_checkpoint(self):
        with open("./articles/checkpoint3", "w") as writer:
    ...
    def save_errors(self, url):
        ...
        import pickle
        with open("./articles/error_urls_3.pkl","wb") as f:
...
import math
index_s = math.floor(len(urls) * (2/4)) # Third part
index_e = math.floor(len(urls) * (3/4)) 
url_keys = list(urls.keys())[index_s:index_e]

with open("./articles/parallel/urls_3.pkl", "wb") as w:
    dmp = url_keys.copy()
    pickle.dump(dmp, w)
    dmp=None
...
urls = tmp.copy()
tmp=None
print("{:,} urls - PARALLEL [3]".format(len(urls.keys())))
