...
class Articles:
    ...
    def save_check(self, savepoint=100):
        if len(self.list) % savepoint == 0:
            import json
            with open("./articles/articles4.json", "w", encoding='UTF-8-sig') as js:
                dmp=self.list.copy()
...
class Crawler:
    ...
    def articles_loader(self):
        import os, json
        if not os.path.exists("./articles/articles4.json"):
            ...
            self.articles = Articles()
            self.articles.list = json.load(open("./articles/articles4.json", "r", encoding="UTF-8-sig"))
            self.checkpoint = int(open("./articles/checkpoint4", "r").read())
    ...
    def save_checkpoint(self):
        with open("./articles/checkpoint4", "w") as writer:
    ...
    def save_errors(self, url):
        ...
        import pickle
        with open("./articles/error_urls_4.pkl","wb") as f:
...
import math
index_s = math.floor(len(urls) * (3/4)) # Last part

url_keys = list(urls.keys())[index_s:]

with open("./articles/parallel/urls_4.pkl", "wb") as w:
    dmp = url_keys.copy()
    pickle.dump(dmp, w)
    dmp=None
...
urls = tmp.copy()
tmp=None
print("{:,} urls - PARALLEL [4]".format(len(urls.keys())))
